...
NX = 4                              # [#] x = x, number
NU = 2                              # [#] a = [accel, steer]
HORIZON_T = 10                              # [#] horizon length                10

R               = np.diag([0.01, 0.01])           # [-] input cost matrix       0.01, 1.0
Rd              = np.diag([0.01, 1.0])           # [-] input difference cost matrix          
Q               = np.diag([1.0, 1.0, 0.5, 10.0])   # [-] state cost matrix      1.0, 1.0, 0.5, 0.5
...
MAX_DSTEER      = np.deg2rad(90.0)  # maximum steering speed [rad/s]            30.0
